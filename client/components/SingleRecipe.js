...
class SingleRecipe extends Component {
  ...
  removeFromFavorites = (evt) => {
    ...
    if (existingFavRecipeIds.includes(recipeIdToBeRemoved)) {
      existingFavRecipeIds.splice(
        existingFavRecipeIds.indexOf(recipeIdToBeRemoved),
        1
      );
    }
  ...
  favoriteButton = () => {
    ...
    if (!existingFavRecipeIds.includes(this.props.recipe.id)) {
      return (
        <Button color="success" onClick={this.addToFavorites}>
          Add To Favorite
        </Button>
      );
    } else {
      return (
        <Button color="warning" onClick={this.removeFromFavorites}>
          ...
          <Card>
            ...
            <CardBody>
              ...
              <CardSubtitle tag="h6" className="mb-2 text-muted">
                Cook time: {readyInMinutes} minutes
              </CardSubtitle>
              <hr />
              <CardText>
                Source: <a href={`${sourceUrl}`}> {sourceUrl} </a>
              </CardText>
              {glutenFree && <CardText>Gluten Free</CardText>}
...
const mapDispatch = (dispatch) => ({
  ...
  addToFavorites: (userId, recipeId) =>
    dispatch(addToFavorites(userId, recipeId)),
  removeFromFavorites: (userId, recipeId) =>
    dispatch(removeFromFavorites(userId, recipeId)),
});
